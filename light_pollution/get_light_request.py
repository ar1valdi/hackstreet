...
def get_light_by_coords(lat,lon):
    ...
    try:
        ...
        str_link = f'https://www.lightpollutionmap.info/QueryRaster/?qk=MTczMjgyOTU0MDY2MTtpc3Vja2RpY2tzOik=&ql=wa_2015&qt=point&qd={lat},{lon}'
        #print(str_link)
        result = requests.get(str_link)
        res_attrs = {}
        res_attrs['light'] = float(str(result._content).split("'")[1].split(",")[0])
        res_attrs['height'] = int(str(result._content).split("'")[1].split(",")[1])
        res_attrs['lat'] = lat
        res_attrs['lon'] = lon
